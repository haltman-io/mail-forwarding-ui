...
export async function fetchApiTokens(
  ...
  return adminRequest<ListResponse<AdminApiToken>>({ path: `/admin/api-tokens?${qs.toString()}` });
}

export async function createApiToken(
  body: { owner_email: string; days: number },
) {
  return adminRequest<CreateUpdateResponse<AdminApiToken>>({ path: "/admin/api-tokens", method: "POST", body });
}
...
export async function updateApiToken(
  ...
  return adminRequest<CreateUpdateResponse<AdminApiToken>>({ path: `/admin/api-tokens/${id}`, method: "PATCH", body });
}
